...
## What's New in v2.9 (Site Visit Notes + Photos + No-Login Subcontractor Share Links)
- **New internal tool for Luis: `/admin/visits`** — a password-protected dashboard (same login/session as `/admin/leads`, now with a 2-tab nav to switch between "Leads" and "Site Visits") for taking structured notes during an in-person site visit, replacing paper notes / memory. Fully isolated from the booking form, chat widget, and lead-notification email flow — nothing in `lead.ts` or `chat-widget.js` was touched.
- **Data captured per visit**: client & property info (name, phone, email, address, city, property type, visit date), services selected (sanding-natural, sanding-with-stain, new hardwood installation, repair, other) with conditional fields that only appear when relevant (stain color + finish coats for custom stain jobs, install type — red oak / prefinished / laminate — for new installations), a **repeatable room-by-room measurement table** (room name, length × width in feet, auto-calculated square footage, current floor type, condition, notes) with a running total square footage, site/technical details (stairs count, access difficulty, furniture-moving responsibility, pets, subfloor condition, color-match notes), and pricing/closing fields (quoted estimate, final price, pricing notes, target start date, interest level, next step, free-form notes).
- **Photo upload, backed by a new Cloudflare R2 bucket (`michael-ai-hardwood-photos`)**: photos can be attached to a visit (optionally tagged to a specific room, with a caption), viewed in a grid on the visit's detail page, and deleted. Only the R2 object key is stored in D1 — the binary photo data lives in R2, never in the database.
- **The headline feature: a public, no-login "share link" for each visit** (`/share/visit/:token`, where `:token` is an unguessable random UUID-based string generated at creation time) — designed specifically so Luis can paste one link into WhatsApp for the subcontractor assigned to that job, and they see the full visit write-up and every photo **without needing any account or password**. The share view deliberately **hides all pricing and internal-notes fields** (quoted estimate, final price, pricing notes) — a subcontractor only sees the job details and photos they need to do the work. An invalid or made-up token returns a plain 404, so links can't be brute-forced or guessed into exposing a different client's job.
- **Security/isolation notes**: session-cookie auth logic was extracted into a shared `src/lib/adminAuth.ts` so `/admin/leads` and `/admin/visits` use one consistent mechanism; the public photo route double-checks that the requested photo actually belongs to the visit matching the given share token before serving it (so one visit's share link can never be used to fetch another visit's photo by guessing an ID).
- **Verified end-to-end in production** on the live domain after deploying: logged in, created a real test visit with a room and a photo, confirmed the photo downloads byte-identical through both the admin route and the public share route, confirmed pricing fields are absent from the public view, confirmed an invalid token 404s, then deleted all test data — `/api/lead/submit` and the public booking flow were re-verified unaffected throughout.
- **Not yet built** (explicitly deferred by Luis to a later phase): a separate client-facing Estimate/Receipt generator, and a subcontractor payment-tracking tool (50% deposit / 50% on completion, profit margin, monthly Google Ads spend for net-profit calculation).

## What's New in v2.8 (Admin Leads Dashboard + Real Mobile Email-Delivery Root Cause Fixed)
- **New read-only, password-protected leads dashboard at `/admin/leads`** (`src/routes/admin.ts`). Gives Luis a database-backed safety net to check for new leads directly, independent of whether the Web3Forms notification email/sound ever arrives — important since Google Ads spend depends on never missing a lead. Protected via HTTP Basic Auth (`ADMIN_PASSWORD` Cloudflare secret, never committed to git). The route only ever runs `SELECT` queries — it cannot insert, update, or delete anything, and is fully isolated from the booking form / email flow. Auto-refreshes every 60 seconds; shows the latest 200 leads with name, phone (tap-to-call), email (tap-to-email), address, service, sq ft, estimate, preferred appointment window, a "📞 CALL NOW" badge for `wants_call_now` leads, and status.
- **Real root cause of the intermittent email-delivery issue identified and fixed: mobile browsers can silently abort the in-flight Web3Forms `fetch()` request when the page is backgrounded, navigated away from, or the screen is locked immediately after tapping "Confirm Request"** — which is exactly what a real customer naturally does right after submitting the form on their phone. Because the lead-save to D1 happens first and always completes, the visitor still saw the "✅ Thank you" message and the lead was never lost — but the notification email could vanish silently with no error. **Fixed by adding `keepalive: true` to the `fetch()` call** in `sendWeb3FormsNotification()` (`chat-widget.js`), which tells the browser to finish sending the request even if the page unloads. Confirmed working in a real-world test (submit, then lock screen / switch apps, exactly like a real customer) — the email arrived successfully.
- **Correction to the v2.7 note below**: the "Web3Forms per-IP rate-limiting" explanation for the earlier intermittent test failures was a guess that didn't hold up — some of those failures were actually caused by testing against the temporary sandbox dev-preview URL (a separate local database, unrelated to production) instead of the real domain, and the remaining real ones were the mobile `fetch()`-abort issue described above, now fixed with `keepalive: true`.
...
| GET | `/admin/login`, POST `/admin/login` | HTML login form (not HTTP Basic Auth — replaced in v2.8 because some mobile browsers don't reliably show the native Basic Auth prompt). On success, sets a `michael_admin_session` cookie (SHA-256 of `ADMIN_PASSWORD`, 30-day expiry, `httpOnly`/`secure`/`SameSite=Lax`, path `/`) and redirects to `/admin/leads`. |
| GET | `/admin/leads` | **Password-protected** (session cookie via `/admin/login`), read-only dashboard listing the latest 200 leads straight from D1. Safety net independent of email delivery — see v2.8 notes above. |
| GET | `/admin/visits` | **Password-protected** (same session cookie), list of all site visits. |
| GET/POST | `/admin/visits/new` | **Password-protected.** Form to create a new site visit (client/property info, services, room-by-room measurements, site details, pricing/notes). |
| GET/POST | `/admin/visits/:id`, `/admin/visits/:id/edit` | **Password-protected.** View/edit an existing visit. |
| POST | `/admin/visits/:id/photo` | **Password-protected.** Multipart photo upload for a visit, stored in the `PHOTOS` R2 bucket; metadata row in `site_visit_photos`. |
| POST | `/admin/visits/:id/photo/:photoId/delete` | **Password-protected.** Deletes a photo from R2 and D1. |
| GET | `/admin/visits/:id/photo/:photoId` | **Password-protected.** Streams a photo from R2 for the admin view. |
| GET | `/share/visit/:token` | **Public, no login required.** Read-only view of one visit's details + photos, for sharing with a subcontractor via WhatsApp. `:token` is an unguessable random string generated at visit creation. Pricing and internal notes are hidden. Invalid/unknown tokens return 404. |
| GET | `/share/visit/:token/photo/:photoId` | **Public, no login required.** Streams a photo from R2, but only if it belongs to the visit matching `:token` (prevents guessing IDs to see another job's photos). |
...
- **D1 tables added in v2.9** (migration `0003_site_visits.sql`): `site_visits` (client/property info, services_json, room total, site details, pricing/closing fields, `share_token` unique random string, timestamps — optionally links back to a `leads` row via `lead_id`), `site_visit_rooms` (one row per measured room, FK → `site_visits.id` on delete cascade), `site_visit_photos` (R2 object key + optional room/caption, FK → `site_visits.id` on delete cascade). Completely separate tables from `leads` — the booking form and its data are untouched.
- **Cloudflare R2 bucket** (`michael-ai-hardwood-photos`, binding `PHOTOS`) — stores the actual photo binary data for site visits. Only the R2 key is stored in D1.
...
3. Second iteration ideas: SMS confirmation, calendar sync for appointment windows, A/B testing hero copy, optionally persist `timeline` to D1 if Luis wants it queryable/reportable later. (Admin leads dashboard with a "wants call now" priority flag — done in v2.8, see `/admin/leads` above.)
...
- **Status**: ✅ **Deployed and live in production on the custom domain** — verified end-to-end (landing page, estimate calculator, lead submission all saving to production D1, v3 chat flow with conversational copy polish, 500 sq ft minimum, and updated Prefinished pricing all live with zero console/JS errors on the custom domain). **Full lead capture + email notification pipeline confirmed working on a real device** (lead saved to D1 + email received with sound on Luis's phone, including with the screen locked immediately after submitting — the `keepalive:true` mobile fix). **Admin leads dashboard live at `/admin/leads`** (password stored as the `ADMIN_PASSWORD` Cloudflare Pages secret — not in git; rotate via `wrangler pages secret put ADMIN_PASSWORD --project-name michael-ai-hardwood` if it ever needs to change). **Site Visits + Photos + Subcontractor Share Links live at `/admin/visits`** (v2.9) — migration `0003_site_visits.sql` applied to production D1, R2 bucket `michael-ai-hardwood-photos` created and bound, verified end-to-end on the live domain (create visit → upload photo → open public `/share/visit/:token` link with no cookie → confirm photo downloads byte-identical and pricing is hidden → invalid token 404s), test data cleaned up afterward.
...
- **Last Updated**: 2026-07-23 (v2.8 — admin leads dashboard added; real root cause of intermittent mobile email delivery found and fixed with `keepalive:true`)
