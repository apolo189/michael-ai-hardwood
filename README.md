# Michael AI — Hardwood Flooring Sales Specialist (MVP v2.8 — Admin Leads Dashboard + Mobile Email-Delivery Fix)
...
| GET | `/admin/leads` | **Password-protected** (HTTP Basic Auth, `ADMIN_PASSWORD` secret), read-only dashboard listing the latest 200 leads straight from D1. Safety net independent of email delivery — see v2.8 notes above. |
...
- **Status**: ✅ **Deployed and live in production on the custom domain** — verified end-to-end (landing page, estimate calculator, lead submission all saving to production D1, v3 chat flow with conversational copy polish, 500 sq ft minimum, and updated Prefinished pricing all live with zero console/JS errors on the custom domain). **Full lead capture + email notification pipeline confirmed working on a real device** (lead saved to D1 + email received with sound on Luis's phone, including with the screen locked immediately after submitting — the `keepalive:true` mobile fix). **Admin leads dashboard live at `/admin/leads`** (password stored as the `ADMIN_PASSWORD` Cloudflare Pages secret — not in git; rotate via `wrangler pages secret put ADMIN_PASSWORD --project-name michael-ai-hardwood` if it ever needs to change).
